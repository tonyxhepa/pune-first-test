<template>
    <form>
        <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 flex flex-col my-2">
            ...
            <button @click.prevent="postJob" class="w-32 transition duration-500 bg-orange-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
                Update
            </button>
        </div>
    </form>
...
<script>
    export default {
        ...
        methods: {
            postJob() {
                axios.post('/api/admin/jobs/' + this.$route.params.jobId, {
            ...
            getJob() {
                axios.get('/api/admin/jobs/' + this.$route.params.jobId)
                    .then(res => {
                        this.job = res.data;
                    })
            },
